...
1)  the `vultRmap` package that can be installed by

``` r
remotes::install_github("patchcervan/vultRmap")
```

and provides the functionality needed to run the analysis scripts,
...
3)  the most recent utilization distribution maps in raster format,
    which can be found in the `rasters` directory.
